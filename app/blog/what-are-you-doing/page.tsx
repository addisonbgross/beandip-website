...
const BlogPost_WhatAreYouDoing = () => (
  <BlogPost
    ...
    content={
      <>
        <p>
          We’ve been working on improving how we save the state of the game. We
          previously wrote{' '}
          <Link
          ...
          >
            about how we save the state of the player’s characters and caravan.
          </Link>{' '}
        ...
        <p>
          ...
          audio, and non-interactable scenery, while each sub-level contains all
          of the things we now save, such as resources worked on and cheeky
          enemies.
        </p>

        <p>
          When a saved game is loaded, the caravan and characters are placed at
          their last location, and then the primary level loads in relation to
          the caravan’s position in space. When the caravan loads, it also
        ...
        <p>
          ...
          The transform (location, rotation, and scale) and resource values for
          all logs produced from harvested trees are saved, allowing the logs to
          be spawned in place upon loading a game. Resources like mineable rocks
          and berry bushes have their remaining resource count and individual
          resource values saved as well. For tracking which trees have been
          harvested, we took the reverse approach; a unique identifier for each
          tree felled is recorded and when a sub-level is loaded, the game
          checks the list of felled tree ids and deletes any trees in the list.
          We use this same approach for rescued elders: since an elder’s bonuses
          are saved in the caravan state, any elders that have been rescued can
          be deleted.
        </p>
        ...
        <p>
          ...
          their state, and can cause a small hitch in game performance. Ideally
          we would only load the state of the levels present at the start of a
          gaming session. We’re currently unable to differentiate these two
          situations when loading a sub-level, but surely after enough coffees
