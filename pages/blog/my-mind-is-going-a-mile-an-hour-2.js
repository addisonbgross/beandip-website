...
export default function BlogPost_MyMindIsGoingAMileAnHourPart2() {
  return (
    <BlogPost
      ...
      content={
        <>
          <p>
            In our last post, we went through how the units in our game
            experience their environment, as well as how they make decisions.
            The actions of a unit are composed of a vector of Jobs, and these
            Jobs are the sub-actions that must be taken to complete a task. We
            work backwards from the desired result to build the vector of Jobs.
            In this post, we will share where these Jobs even come from and how
            they influence the animation state of each unit.
          </p>
          ...
          <p>
            ...
            the unit or by dragging the selection shape over them. Once units
            have been selected, the player then uses the right mouse button to
            assign Jobs. Enemies, resources, and anything else that the units
            can interact with implement an interface named Job Factory, which is
          ...
          <p>
            ...
            however, this is a unique case as the landscape is not a Job
            Factory.
          </p>

          <p>
            The player will receive the Job from the relevant Job Factory and
            disperse it to the selected unit(s). The unit’s AI will then
            generate the vector of Jobs needed to complete the original job
            based on the unit’s current state. “Current State” is a bit of a
            misnomer as there is not a lot of state stored in each unit. Aside
            from configuration parameters such as vision range or team
            identifier, units only store their currently visible friends and
          ...
          <p>
            No one likes having to be extremely precise when selecting units in
            a RTS game! We’ve put a lot of effort into making the player’s
            clicking experience as enjoyable as possible, and we aim to produce
            predictable clicking results.
          </p>
          ...
          <p>
            ...
            are located behind trees, so a left click will go through the tree
            and select the unit.However, when the player right clicks on a tree,
            the tree needs to produce a Chop Wood job. Since there is usually
            landscape behind the tree, and we mentioned in the previous section
            that clicking on the landscape produces a Move job, how do we know
            to assign the Chop Wood job and not the Move job?
          </p>

          <p>
            To decide which Job is assigned based on clicks that pass through
            more than one thing, we shoot a ray from the game camera to the
            mouse cursor. The ray will travel through things that will allow it
            to pass such as trees and buildings, but will stop when it
            encounters something that blocks it like a unit or the landscape.
            After the ray is stopped, we collect the entities that the ray has
            passed through and take a look at the first thing in the list. If
            this first thing is a Job Factory we take a Job. If the first thing
            is not a Job Factory, we continue to go through the list of ray
            collided entities until we find a valid job. If no valid job is
            found, then the landscape must be the final, blocking entity. This
            sorting process caused a fair amount of headache, but the result is
            that the player will always be able to select their units no matter
            where they are, and they will always be able to instruct their units
            as expected.
          </p>
          ...
          <p>
            When a unit has its Job vector sorted and is ready to take action,
          ...
          <p>
            The Movement Component uses the navigation mesh, which is produced
            by the Caravan, to find the quickest path to the desired location.
            This navigation mesh is generated in a large radius centered on the
          ...
          <p>
            When a unit is not in motion, nor performing a Job, they will show
            an idle animation. But when the unit moves or has something
            interesting to do, we must decide on which animations to show them
            doing. Most Jobs have unique animations, such as:
          </p>
          ...
          <p>
            ...
            use the Main state machine for performing resource gathering jobs
            and attacking enemies if nothing dire is occurring, but if something
          ...
          <p>
            ...
            animations are for things like when a unit gets stunned by their
            foe. Below we show the Main state machine and the sub-state machines
            that are used for the different scenarios.
          </p>
