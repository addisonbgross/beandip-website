...
export default function BlogPost_UnlimitedJuice() {
  return (
    <BlogPost
      ...
      content={
        <>
          ...
          <p>
            Units can be upgraded to have both passive and action skills.
            Passive skills increase a unit’s stats in some way, while action
            skills allow a unit to perform a special action when triggered. A
            unit’s action skills will be triggered under circumstances such as:
            seeing an enemy, seeing an ally, attacking an enemy, or receiving a
          ...
          <p>
            ...
            having only 3 actions skills in use at a time makes the player have
            to think about how their units are configured depending on their
            current situation. For example when battling a group of Archers, the
            Cadet’s Chest Kick skill may not be the best choice as it will send
            enemies flying backwards. This will just create more distance
            between the Cadet and their ranged attacking foes. A counter example
            would be the Miner’s Poor Sport action skill. This skill sends the
            Miner into a rage when hit by ranged attacks, causing the Miner to
            move faster for a few seconds. This would help the melee weapon
            wielding Miner close the distance between themself and the ranged
            Archers.
          </p>
          ...
          <p>
            Action skills can be configured with at least one special effect,
            and we can specify if an effect occurs when the skill starts or when
            it completes. How we configure an action skill can be represented
          ...
          <p>
            To keep things interesting, some action skills use unique
            animations. These animations will use the Action Skill Response to
            trigger special effects just like a skill that doesn’t use a
            specific animation.
          </p>
